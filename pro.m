...
% Get a binary image of the a region, and pad it to avoid boundary effects during the stroke width computation.
...
while 1
    userPrompt = 'Do you want to say anything to matlab?';
    ...
    if strcmp(char(caUserInput),'exit')
       break 
    end
    
    if strcmp(char(caUserInput),'repeat')
        sentence = [ocrtxt.Text];
        try
            NET.addAssembly('System.Speech');
            Speaker = System.Speech.Synthesis.SpeechSynthesizer;
    ...
    fid1 = fopen('input.csv','r'); 
    data=[];
    while ~feof(fid1)
        line = fgets(fid1);
        C = strsplit(line,',');
        if strcmp(char(caUserInput),C{1}) 
            NET.addAssembly('System.Speech');
